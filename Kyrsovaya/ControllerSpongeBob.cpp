...
void AControllerSpongeBob::Tick(float DeltaTime) {
	...
	if ((TPC != NULL) && (TPC->RandHod==false) && (TPC->Winner == false)&&(TPC->sort)) {
		...
		//	Check = MoveComplete();
			if (OldPozition == TPC->VseBub.Num() - 1 && MoveComplete()) { //если позиция равна последней то Губка победил 
			...
			else if (OldPozition == NewPozition-1 && MoveComplete()) { //если они равны, значит Sponge пришел на нужное место
				TPC->RandKub = 0;
